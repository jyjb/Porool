 ...
 *
 * Build:
 *   gcc scr/porool_cli.c -std=c99 -Iinclude -Llib -lporool -ltharavu_dll \
 *       -lm -o bin/porool.exe
 */

#include "porool.h"
#include "tharavu_dll.h"   /* stats / peek / phrasing-list use tde directly */
...
static void ensure_porool_ini(const char *path)
{
    ...
    fprintf(fp,
        ...
        "w3 = 0.20\n\n"
        "[source_weights]\n"
        "; Per-source priority multipliers.  Range [0.0, 2.0]\n"
    );
    fclose(fp);
