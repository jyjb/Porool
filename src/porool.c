 ...
 *   host: tde_config_load()  →  porool_init() → ve_init()  →  open chunk table + vector store
 *
...
#ifdef _WIN32
static SRWLOCK          g_phrase_lock = SRWLOCK_INIT;
#  define PHRASE_WLOCK()   AcquireSRWLockExclusive(&g_phrase_lock)
#  define PHRASE_WUNLOCK() ReleaseSRWLockExclusive(&g_phrase_lock)
#  define PHRASE_RLOCK()   AcquireSRWLockShared(&g_phrase_lock)
#  define PHRASE_RUNLOCK() ReleaseSRWLockShared(&g_phrase_lock)
#else
#  include <pthread.h>
static pthread_rwlock_t g_phrase_lock = PTHREAD_RWLOCK_INITIALIZER;
#  define PHRASE_WLOCK()   pthread_rwlock_wrlock(&g_phrase_lock)
#  define PHRASE_WUNLOCK() pthread_rwlock_unlock(&g_phrase_lock)
#  define PHRASE_RLOCK()   pthread_rwlock_rdlock(&g_phrase_lock)
#  define PHRASE_RUNLOCK() pthread_rwlock_unlock(&g_phrase_lock)
#endif
...
static int load_config(const char *path)
{
    ...
    g_cfg.vocab_name[0]       = '\0';
    g_cfg.vocab_group[0]      = '\0';
    g_cfg.vocab_first_lang[0] = '\0';
    strncpy(g_cfg.chunks_table,  "porool.chunks", sizeof(g_cfg.chunks_table)  - 1);
    strncpy(g_cfg.vectors_table, "porool.chunks", sizeof(g_cfg.vectors_table) - 1);
    ...
    int vocab_name_explicit = 0;
    int vocab_group_found   = 0;

    char line[1024], section[64] = "";
    while (fgets(line, sizeof(line), fp)) {
        char *c = strchr(line, ';'); if (!c) c = strchr(line, '#'); if (c) *c = '\0';
        line[strcspn(line, "\r\n")] = '\0';
        trim_inplace(line);
        if (!line[0]) continue;
        ...
        if (strcmp(section, "porool") == 0) {
            ...
            else if (!strcmp(key, "ctx_min_score"))
                g_cfg.ctx_min_score = (float)strtod(val, NULL);
            ...
            else if (!strcmp(key, "vocabularies"))
                snprintf(g_cfg.vocab_group, sizeof(g_cfg.vocab_group), "%.127s", val);
            else
                fprintf(stderr, "porool: warning: [porool] unknown key '%s' — ignored\n", key);
            ...
            if      (!strcmp(key, "w1")) g_cfg.w1 = (float)strtod(val, NULL);
                ...
                fprintf(stderr, "porool: warning: [scoring] unknown key '%s' — ignored\n", key);
        ...
        } else if (g_cfg.vocab_group[0] && strcmp(section, g_cfg.vocab_group) == 0) {
            vocab_group_found = 1;
            if (!g_cfg.vocab_first_lang[0])
                snprintf(g_cfg.vocab_first_lang, sizeof(g_cfg.vocab_first_lang), "%.127s", key);
        }
    }
     ...
     * Priority: explicit vocab_name > derived from vocabularies group > error.
     * Silent fallback to a hardcoded name is not allowed when a config file
     * exists — the caller must know if the vocabulary could not be resolved. */
    if (vocab_name_explicit) {
        if (g_cfg.vocab_group[0])
            fprintf(stderr,
                "porool: note: explicit 'vocab_name = %s' overrides 'vocabularies = %s'\n",
                g_cfg.vocab_name, g_cfg.vocab_group);
    } else if (g_cfg.vocab_group[0]) {
        if (!vocab_group_found) {
            fprintf(stderr,
                "porool: error: 'vocabularies = %s' but section [%s] not found in %s\n",
                g_cfg.vocab_group, g_cfg.vocab_group, path);
            return -1;
        }
        if (!g_cfg.vocab_first_lang[0]) {
            fprintf(stderr,
                "porool: error: [%s] section has no language entries in %s\n",
                g_cfg.vocab_group, path);
            return -1;
        }
        snprintf(g_cfg.vocab_name, sizeof(g_cfg.vocab_name),
                 "%.127s.%.127s", g_cfg.vocab_group, g_cfg.vocab_first_lang);
    } else {
        fprintf(stderr,
            "porool: warning: no 'vocab_name' or 'vocabularies' in %s; "
            "defaulting to 'porool.vocab'\n", path);
        strncpy(g_cfg.vocab_name, "porool.vocab", sizeof(g_cfg.vocab_name) - 1);
    }

    if (g_cfg.w1 + g_cfg.w2 + g_cfg.w3 > 1.001f)
        fprintf(stderr,
            "porool: warning: w1+w2+w3 = %.3f; downstream normalisation assumes sum <= 1.0\n",
            g_cfg.w1 + g_cfg.w2 + g_cfg.w3);
    return 0;
}
...
POROOL_API int porool_init(const char *config_path)
{
    ...
    /* Host must call tde_config_load() before porool_init(). */
    /* 1. Init Sorkuvai vocabulary engine — THARAVU already configured by host */
    if (ve_init(g_cfg.vocab_name, NULL) != VE_OK) return -3;

    /* 2+3. Open default chunk/vector tables — non-fatal if absent.
    ...
    /* 4. Ensure phrasing tables exist then load into cache */
...
{
    return g_ready ? sk_get_dim() : 0;
}

/* Re-open the default chunk/vector handles so queries see an updated index.
 * Call after an external vocabulary reload (Sorkuvai) or index rebuild.
                                          ...
                                          int   *result_count)
{
    ...
    *result_count = 0;
    ...
    /* Catch dimension mismatch between a rebuilt vocab and stale index.
     * Call porool_refresh() after a Sorkuvai vocab reload to resync. */
    if (tde_row_count(g_chunks) > 0) {
        uint32_t stored_dim = 0;
        tde_vector_get_ptr(g_vectors, 0, &stored_dim);
                                      ...
                                      int max_chars, float ctx_min_score)
{
    ...
    for (int i = 0; i < count; i++) {
        /* Skip chunks below minimum score threshold (R31). */
...
POROOL_API char *porool_query(const char *query, int top_k, int max_chars)
{
    ...
    char *ctx = porool_build_context(res, n, max_chars, g_cfg.ctx_min_score);
                                        ...
                                        const ChunkMeta *meta)
{
    ...
    /* language: explicit tag wins; fall back to first configured language */
    const char *m_language = (meta && meta->language && meta->language[0])
                             ? meta->language
                             : g_cfg.vocab_first_lang;
...
#define MAX_TABLES 64
    ...
    for (int t = 0; t < ntables && total_found < total_cap; t++) {
        ...
        /* Skip tables whose stored dimension doesn't match the current query dim. */
