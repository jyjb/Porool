...
static porool_t *g_p = NULL;   /* shared instance across all test sections */
...
static void test_pre_init(void)
{
    ...
    CHECK("refresh(NULL) returns -1", porool_refresh(NULL) == -1);
}
...
static void test_init(void)
{
    ...
    g_p = porool_create(&pcfg);
    CHECK("porool_create returns non-NULL", g_p != NULL);
}

static void test_ingest(void)
{
    printf("\n[ingest]\n");
    if (!g_p) { printf("  SKIP  (no instance)\n"); return; }
...
static void test_embed_query(void)
{
    ...
    float *emb = NULL;
    int    dim  = 0;
    int    r    = porool_embed_query(g_p, "what is NeRF", &emb, &dim);
...
static void test_retrieve_rerank(void)
{
    ...
    float *emb = NULL;
    int    dim  = 0;
    if (porool_embed_query(g_p, "neural radiance field volume density",
                            &emb, &dim) != 0 || !emb) {
    ...
    int           count   = 0;
    SearchResult *results = porool_retrieve(g_p, emb, 5, &count);
    porool_free((char *)emb);
    ...
    porool_rerank_query(g_p, results, count, "what is neural radiance field");
...
static void test_build_context(void)
{
    ...
    float *emb = NULL;
    int    dim  = 0;
    if (porool_embed_query(g_p, "generative model gaussian noise latent",
                            &emb, &dim) != 0 || !emb) {
        printf("  SKIP  (embed_query failed)\n");
        return;
    }
    ...
    char *ctx = porool_build_context(g_p, results, count, max_chars, 0.0f);
    porool_free_results(results, count);
...
static void test_query(void)
{
    ...
    char *ctx = porool_query(g_p, "what is a diffusion model", 5, 800);
    ...
    char *ctx2 = porool_query(g_p, "NeRF rendering", 0, 0);
...
static void test_query_from(void)
{
    ...
    char *ctx = porool_query_from(g_p, "NeRF novel view synthesis",
                                   "test_p", "chunks", 5, 800);
    ...
    char *ctx2 = porool_query_from(g_p, "volumetric rendering neural",
                                    "test_p", NULL, 5, 800);
...
static void test_query_target(void)
{
    ...
    char *r1 = porool_query_target(g_p, "diffusion model generative noise",
                                    "ALL", 5, 800);
    CHECK("query_target ALL returns non-NULL", r1 != NULL);
    if (r1) porool_free(r1);

    char *r2 = porool_query_target(g_p, "radiance field view synthesis",
                                    "test_p", 5, 800);
    CHECK("query_target db returns non-NULL", r2 != NULL);
    if (r2) porool_free(r2);

    char *r3 = porool_query_target(g_p, "volumetric density rendering",
                                    "test_p.chunks", 5, 800);
...
static void test_synonym(void)
{
    ...
    char *ctx = porool_query(g_p, "nerf", 3, 400);
...
static void test_refresh(void)
{
    ...
    int r = porool_refresh(g_p);
    CHECK("refresh after init returns 0", r == 0);

    char *ctx = porool_query(g_p, "NeRF volume density neural network", 3, 400);
...
static void test_sorkuvai_reload_pipeline(void)
{
    ...
    int r2 = porool_refresh(g_p);
    CHECK("porool_refresh after vocab reload returns 0", r2 == 0);

    char *ctx = porool_query(g_p, "diffusion model latent stable", 3, 400);
...
static void test_destroy(void)
{
    ...
    porool_destroy(g_p);
    g_p = NULL;
    CHECK("destroy does not crash", 1);

    /* After destroy a NULL instance must refuse further operations */
    CHECK("refresh(NULL) after destroy returns -1", porool_refresh(NULL) == -1);
}
...
int main(void)
{
    ...
    write_test_ini("test_porool.sxp");
